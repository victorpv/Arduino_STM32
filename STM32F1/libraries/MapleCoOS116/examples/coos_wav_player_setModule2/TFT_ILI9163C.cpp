...
void TFT_ILI9163C::writecommand(uint8_t c){
    ...
    tftSPI.beginTransaction(SPISettings(SPICLOCK, MSBFIRST, SPI_MODE0));
    #endif
...
void TFT_ILI9163C::setBitrate(uint32_t n){
	...
	if (n >= 8000000) {
		...
		tftSPI.setClockDivider(SPI_CLOCK_DIV4);
	} else if (n >= 2000000) {
		tftSPI.setClockDivider(SPI_CLOCK_DIV8);
	} else {
		tftSPI.setClockDivider(SPI_CLOCK_DIV16);
	}
	#endif
}
...
void TFT_ILI9163C::setBitrate(uint32_t n){
	...
	tftSPI.setClockDivider(divider);
	#endif
}
...
void TFT_ILI9163C::writedata(uint8_t c){
	...
	*csport &= ~cspinmask;
	...
	tftSPI.setDataSize (0);
#endif
	spiwrite(c);
#if defined SPI_16BIT
	tftSPI.setDataSize (SPI_CR1_DFF);
#endif
	*csport |= cspinmask;
	#ifdef SPI_HAS_TRANSACTION
	tftSPI.endTransaction();
...
void TFT_ILI9163C::begin(void) {
	...
	#endif
	#if !defined (SPI_HAS_TRANSACTION)
    tftSPI.setClockDivider(SPI_CLOCK_DIV2); // 8 MHz
	...
	*csport &= ~cspinmask;
    ...
    tftSPI.begin();
	#if !defined (SPI_HAS_TRANSACTION)
    tftSPI.setClockDivider(11); // 8 MHz
    tftSPI.setBitOrder(MSBFIRST);
    tftSPI.setDataMode(SPI_MODE0);
	#endif
	// toggle RST low to reset; CS low so it'll listen to us
...
#elif defined(__MK20DX128__) || defined(__MK20DX256__)
	tftSPI.begin();
	if (tftSPI.pinIsChipSelect(_cs, _rs)) {
		pcs_data = tftSPI.setCS(_cs);
		pcs_command = pcs_data | tftSPI.setCS(_rs);
	} else {
		pcs_data = 0;
		pcs_command = 0;
		return;
...
void TFT_ILI9163C::writeScanline(size_t n) {
	if (n == 0) return;
#if defined SPI_16BIT
    tftSPI.dmaSend(_scanlineBuffer16, n, 0);	// each pixel is one halfword. circular mode
#else
    tftSPI.dmaSend(_scanlineBuffer8, n << 1);	// each pixel is 2 bytes
#endif
}
