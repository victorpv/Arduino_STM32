...
#include <libmaple/timer.h>
	...
	extern timer_dev * volatile TMRpcm_dev = TIMER1;
	...
	extern volatile char TMRpcm_volMod=0;
...
/*				sFile.read((byte*)TMRpcm_read_buffer,512);
				for (uint16 n = 0; n < 512; n++){ // Fill buffer for both channels with the same information.
...
void TMRpcm_finalizeWavTemplate(char* filename){
	...
	TMRpcm_seek(4); byte data[4] = {lowByte(fSize),highByte(fSize), fSize >> 16,fSize >> 24};
	sFile.write(data,4);
	byte tmp;
	TMRpcm_seek(40);
	fSize = fSize - 36;
